...
def mc_run_parser(run_dir):
    """Wrapper for read_mc_results_file that enables compatibility with the gridspace manager.
    ...
    -------
    dict
        Dictionary of results from the monte carlo run.
    """
    return read_mc_results_file(os.path.join(run_dir, "results.json"))


def mc_LTE_run_parser(run_dir):
    """Wrapper for read_mc_results_file that enables compatibility with the gridspace manager.
    ...
    dict
        Dictionary of results from the monte carlo run.
    """
    return read_lte_results(os.path.join(run_dir, "results.json"))
