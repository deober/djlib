...
for training_dir in open(training_dir_path_list):
    ...
    for config in open(os.path.join(training_dir.strip(), 'configs_list.txt')):
        ...
        vasputils.setup_scan_calculation_from_existing_run(config,training_dir,hours=10,slurm=True,run_jobs=False,delete_submit_script=False,max_relax_steps=20)
